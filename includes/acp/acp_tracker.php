...
class acp_tracker
{
	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			...
			case 'severity':
				$this->manage_severity($action);
			break;
			
			case 'priority':
				$this->manage_priority($action);
			break;
		...
		switch ($action)
		{
			case 'delete':
				...
				{								
					...
					confirm_box(false, 'ACP_TRACKER_ATTACHMENTS_DELETE', build_hidden_fields($s_hidden_fields));
				}				
			break;
	...
	function manage_project($action)
	{
		...
		switch ($action)
		{
			case 'delete':
				...
				confirm_box(false, 'ACP_TRACKER_PROJECT_DELETE', build_hidden_fields($s_hidden_fields));	
			break;
	...
	function manage_severity($action)
	{
		global $db, $user, $auth, $template, $cache, $mode;
		global $config, $phpbb_admin_path, $phpbb_root_path, $phpEx;
		...
		$form_key = 'acp_tracker';
		add_form_key($form_key);
		$this->page_title = 'ACP_TRACKER_SEVERITY';

		$project_id = request_var('project_id', 0);
		$severity_id = request_var('severity_id', 0);
		$submit	= (isset($_POST['submit'])) ? true : false;
		
		$projects = $tracker->get_projects();
		
		$template->assign_vars(array(
			'S_IN_MANAGE_SEVERITY'		=> true,
		));
		...
		switch ($action)
		{
			case 'add':
				if ($submit)
				{
					...
					$severity_data = array(
						'severity_name'			=> utf8_normalize_nfc(request_var('severity_name', '', true)),
						'project_id'			=> $project_id,
					);
					
					$tracker->handle_project_items('add', $mode, $severity_data);
					add_log('admin', 'LOG_TRACKER_SEVERITY_ADD', $severity_data['severity_name']);
					trigger_error($user->lang['ACP_TRACKER_SEVERITY_ADDED'] . adm_back_link($this->u_action . "&amp;action=view&amp;project_id=$project_id"));
				}
				else
				{
					$severity_data = array();
			...
			case 'edit':
				...
				{				
					...
					$tracker->handle_project_items('update', $mode, $severity_data, $severity_id);
					add_log('admin', 'LOG_TRACKER_SEVERITY_EDIT', $severity_data['severity_name']);
					trigger_error($user->lang['ACP_TRACKER_SEVERITY_EDITED'] . adm_back_link($this->u_action . "&amp;action=view&amp;project_id=$project_id"));
				}
				else
				{				
					$sql = 'SELECT *
							FROM ' . TRACKER_SEVERITY_TABLE . '
			...
			case 'delete':		
				...
				{				
					...
					if ($row)
					{
						$tracker->handle_project_items('delete', $mode, false, $severity_id);
						add_log('admin', 'LOG_TRACKER_SEVERITY_DELETE', $row['severity_name']);
						trigger_error($user->lang['ACP_TRACKER_SEVERITY_DELETED'] . adm_back_link($this->u_action . "&amp;action=view&amp;project_id=$project_id"));
					}
					else
					{
						trigger_error($user->lang['ACP_TRACKER_SEVERITY_NO_ID'] . adm_back_link($this->u_action .  "&amp;action=view&amp;project_id=$project_id"), E_USER_WARNING);	
					}
				}
			break;
		...
		switch ($action)
		{
			case 'delete':
				...
				confirm_box(false, 'ACP_TRACKER_SEVERITY_DELETE', build_hidden_fields($s_hidden_fields));	
			break;
			...
			case 'add':
				if ($action == 'add')
				{
					$severity_data['severity_name'] = '';
				}
				
				$template->assign_vars(array(
					'S_IN_MANAGE_SEVERITY_ADD_EDIT'		=> true,
					'U_BACK'							=> $this->u_action,
					'U_ACTION'							=> $this->u_action . '&amp;action=' . $action,
					
					'SEVERITY_NAME'						=> $severity_data['severity_name'],					
					'PROJECT_ID'						=> $project_id,
				));
			...
			case 'view':
				$template->assign_vars(array(
					'S_IN_MANAGE_SEVERITY_VIEW'	=> true,
					'PROJECT_ID'				=> $project_id,
					'U_ACTION' 					=>  "{$this->u_action}&amp;action=add",
				));				
				...
				foreach ($row as $item)
				{
						$template->assign_block_vars('severity', array(
							'SEVERITY_NAME'		=>  $tracker->set_lang_name($item['severity_name']),
							'U_EDIT' 			=> "{$this->u_action}&amp;action=edit&amp;severity_id={$item['severity_id']}&amp;project_id={$item['project_id']}",
							'U_DELETE' 			=> "{$this->u_action}&amp;action=delete&amp;severity_id={$item['severity_id']}&amp;project_id={$item['project_id']}",
		...
		$template->assign_vars(array(
			'S_IN_MANAGE_SEVERITY_DEFAULT'	=> true,
			'S_PROJECT_OPTIONS'				=> $tracker->project_select_options($projects, false, $mode),
			'U_ACTION' 						=>  "{$this->u_action}&amp;action=view",
		));	
	...
	function manage_priority($action)
	{
		global $db, $user, $auth, $template, $cache, $mode;
		global $config, $phpbb_admin_path, $phpbb_root_path, $phpEx;
		...
		$form_key = 'acp_tracker';
		add_form_key($form_key);
		$this->page_title = 'ACP_TRACKER_PRIORITY';

		$project_id = request_var('project_id', 0);
		$priority_id = request_var('priority_id', 0);
		$submit	= (isset($_POST['submit'])) ? true : false;
		
		$projects = $tracker->get_projects();
		
		$template->assign_vars(array(
			'S_IN_MANAGE_PRIORITY'		=> true,
		));
		...
		switch ($action)
		{
			case 'add':
				if ($submit)
				{
					...
					$priority_data = array(
						'priority_name'			=> utf8_normalize_nfc(request_var('priority_name', '', true)),
						'project_id'			=> $project_id,
					);
					
					$tracker->handle_project_items('add', $mode, $priority_data);
					add_log('admin', 'LOG_TRACKER_PRIORITY_ADD', $priority_data['priority_name']);
					trigger_error($user->lang['ACP_TRACKER_PRIORITY_ADDED'] . adm_back_link($this->u_action . "&amp;action=view&amp;project_id=$project_id"));
				}
				else
				{
					$priority_data = array();
			...
			case 'edit':
				...
				{				
					...
					$tracker->handle_project_items('update', $mode, $priority_data, $priority_id);
					add_log('admin', 'LOG_TRACKER_PRIORITY_EDIT', $priority_data['priority_name']);
					trigger_error($user->lang['ACP_TRACKER_PRIORITY_EDITED'] . adm_back_link($this->u_action . "&amp;action=view&amp;project_id=$project_id"));
				}
				else
				{				
					$sql = 'SELECT *
							FROM ' . TRACKER_PRIORITY_TABLE . '
			...
			case 'delete':		
				...
				{				
					...
					if ($row)
					{
						$tracker->handle_project_items('delete', $mode, false, $priority_id);
						add_log('admin', 'LOG_TRACKER_PRIORITY_DELETE', $row['priority_name']);
						trigger_error($user->lang['ACP_TRACKER_PRIORITY_DELETED'] . adm_back_link($this->u_action . "&amp;action=view&amp;project_id=$project_id"));
					}
					else
					{
						trigger_error($user->lang['ACP_TRACKER_PRIORITY_NO_ID'] . adm_back_link($this->u_action .  "&amp;action=view&amp;project_id=$project_id"), E_USER_WARNING);	
					}
				}
			break;
		...
		switch ($action)
		{
			case 'delete':
				...
				confirm_box(false, 'ACP_TRACKER_PRIORITY_DELETE', build_hidden_fields($s_hidden_fields));	
			break;
			
			case 'edit':
				$template->assign_vars(array(
					'PRIORITY_ID'				=> $priority_data['priority_id'],
				));
			case 'add':
				if ($action == 'add')
				{
					$priority_data['priority_name'] = '';
				}
				
				$template->assign_vars(array(
					'S_IN_MANAGE_PRIORITY_ADD_EDIT'		=> true,
					'U_BACK'							=> $this->u_action,
					'U_ACTION'							=> $this->u_action . '&amp;action=' . $action,
			...
			case 'view':
				$template->assign_vars(array(
					'S_IN_MANAGE_PRIORITY_VIEW'	=> true,
					'PROJECT_ID'				=> $project_id,
					'U_ACTION' 					=>  "{$this->u_action}&amp;action=add",
				));				
				...
				foreach ($row as $item)
				{
						$template->assign_block_vars('priority', array(
							'PRIORITY_NAME'		=>  $tracker->set_lang_name($item['priority_name']),
							'U_EDIT' 			=> "{$this->u_action}&amp;action=edit&amp;priority_id={$item['priority_id']}&amp;project_id={$item['project_id']}",
							'U_DELETE' 			=> "{$this->u_action}&amp;action=delete&amp;priority_id={$item['priority_id']}&amp;project_id={$item['project_id']}",
		...
		$template->assign_vars(array(
			'S_IN_MANAGE_PRIORITY_DEFAULT'	=> true,
			'S_PROJECT_OPTIONS'				=> $tracker->project_select_options($projects, false, $mode),
			'U_ACTION' 						=>  "{$this->u_action}&amp;action=view",
		));	
